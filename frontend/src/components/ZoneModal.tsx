...
import { statLabels, FRAGMENT_ICON, XGEN_ICON } from '../lib/stats'
                    ...
                    >
                      {itemInfo?.icon && (itemInfo.icon.startsWith('http') || itemInfo.icon.startsWith('/')) ? (
                        ...
                        <span className="text-lg">
                          {(itemInfo?.icon && itemInfo.icon.length > 2) ? '📦' : (itemInfo?.icon || (dropConfig.emoji === FRAGMENT_ICON ? <FragmentIcon className="h-5 w-5" /> : dropConfig.emoji === XGEN_ICON ? <XGenIcon className="h-5 w-5" /> : dropConfig.emoji))}
                        </span>
                      )}
                    </div>
