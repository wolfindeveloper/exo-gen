...
export default function ShipPage() {
  ...
  const activeZoneName = activeExp
    ? (zonesContent.find((z) => z.id === activeExp.zone_id)?.name ?? activeExp.zone_id)
    : null
  const expTimer = useExpeditionTimer(activeExp?.started_at ?? null, activeExp?.ends_at ?? null)
              ...
              <div>
                ...
                <div className="h-2 bg-black/40 rounded-full overflow-hidden border border-orange-500/10">
                  <div
                    ...
                    style={{ width: `${Math.min(100, ((mainShip?.tea_level ?? 50) / 100) * 100)}%`, boxShadow: '0 0 6px rgba(249,115,22,.3)' }}
                  />
                </div>
                {(mainShip?.tea_level ?? 0) < 100 && (
                  <button
              ...
              <div>
                ...
                <div className="h-2 bg-black/40 rounded-full overflow-hidden border border-green-500/10">
                  <div
                    ...
                    style={{ width: `${Math.min(100, ((mainShip?.optimism ?? 85) / 100) * 100)}%`, boxShadow: '0 0 6px rgba(34,197,94,.3)' }}
                  />
                </div>
                {(mainShip?.optimism ?? 0) < 100 && (
                  <button
